...
public class IO
{
		...
		try
		{
			try
			{
				try
				{
					try
					{
						var file = new FileInfo(P_0);
						if (!file.Exists)
						{
							var text = ("File ") + (P_0) + (" not found.");
							//
						...
						if (d.Exists)
						{
							var text2 = (P_0)+(" is a directory. Please use a valid file name.");
							//
							throw new JJTreeIOException(text2);
						}
						if (JavaCCGlobals.isGeneratedBy("JJTree", P_0))
						{
							var text3 = (P_0)+(" was generated by jjtree.  Cannot run jjtree again.");
							//
	...
	IL_00e1:
		...
		string text6 = "File " + P_0 + " not found.";
		//
		throw new JJTreeIOException(text6);
	IL_00de:

		string text7 = ("Security violation while trying to open ")+(P_0);
		//
