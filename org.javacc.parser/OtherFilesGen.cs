...
public class OtherFilesGen : JavaCCParserConstants //JavaCCGlobals, 
{
	...
	public static void start()
	{
		...
		ostr.WriteLine(" */");
		ostr.WriteLine(new StringBuilder().Append("public interface ").Append(JavaCCGlobals.cu_name).Append("Constants {")
			.ToString());
		ostr.WriteLine("");
		...
		while (enumeration.hasMoreElements())
		{
			...
			ostr.WriteLine(new StringBuilder().Append("  int ").Append(regularExpression.label).Append(" = ")
				.Append(regularExpression.ordinal)
				.Append(";")
				.ToString());
		}
		ostr.WriteLine("");
		if (!Options.getUserTokenManager() && Options.getBuildTokenManager())
		{
			for (int j = 0; j < (nint)LexGen.lexStateName.LongLength; j++)
			{
				...
				ostr.WriteLine(new StringBuilder().Append("  int ").Append(LexGen.lexStateName[j]).Append(" = ")
					.Append(j)
					.Append(";")
					.ToString());
			}
		...
		while (enumeration.hasMoreElements())
		{
			...
			while (enumeration2.hasMoreElements())
			{
				...
				{
					ostr.WriteLine(new StringBuilder().Append("    \"\\\"").Append(JavaCCGlobals.add_escapes(JavaCCGlobals.add_escapes(((RStringLiteral)regularExpression).image))).Append("\\\"\",")
						.ToString());
					continue;
				}
				if (!string.Equals(regularExpression.label, ""))
				{
					ostr.WriteLine(new StringBuilder().Append("    \"<").Append(regularExpression.label).Append(">\",")
						.ToString());
					continue;
				}
				...
				ostr.WriteLine(new StringBuilder().Append("    \"<token of kind ").Append(regularExpression.ordinal).Append(">\",")
					.ToString());
			}
		}
		ostr.WriteLine("  };");
		...
		JavaCCErrors.Semantic_Error(new StringBuilder().Append("Could not open file ").Append(JavaCCGlobals.cu_name).Append("Constants.java for writing.")
			.ToString());
